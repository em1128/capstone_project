...
{  
	...
	const int32_t TIMEOUT_IN_MS = 100000000; //for camera capture timeout
    ...
    do
    {
        ...
        if (get_capture_result == K4A_WAIT_RESULT_SUCCEEDED)
        {
            ...
            if (pop_frame_result == K4A_WAIT_RESULT_SUCCEEDED)
            {
                ...
                k4a_transformation_t trans = k4a_transformation_create(&sensor_calibration);
                k4a_image_t a;
                ...
                Mat rod_l[56];//rodrigues vector local
                Mat rod_g[56]; //rodrigues vector  global
                ...
                if (num_bodies) {
                    ...
                    k4a_image_create(K4A_IMAGE_FORMAT_CUSTOM, 640, 576, 3840, &a);
                    if (k4a_transformation_depth_image_to_point_cloud(trans, depth, K4A_CALIBRATION_TYPE_DEPTH, a) == K4A_RESULT_SUCCEEDED) {
                        cout << "1" << '\n';
                    }
                    else {
                        cout << "0" << '\n';
                    ...
                    uint8_t* depth_buffer = k4a_image_get_buffer(a);
                    cv::Mat depthMat(576, 1280, CV_8UC1, (void*)depth_buffer, cv::Mat::AUTO_STEP); // color camera 이미지
                    ...
                    //// body traking image showing
                    //cv::imshow("image", depthMat);
                    //cv::imwrite("image.png", depthMat); // "image"라는 이름의 창에 이미지를 넣어 보여줌
                ...
                k4a_transformation_destroy(trans);
