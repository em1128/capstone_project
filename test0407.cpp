        ...
        if (get_capture_result == K4A_WAIT_RESULT_SUCCEEDED)
        {
            ...
            if (pop_frame_result == K4A_WAIT_RESULT_SUCCEEDED)
            {
                ...
                for (size_t i = 0; i < num_bodies; i++)
                {
                    ...
                    for (int j = 0; j <= 27; j++) {
                        if (j == 9 || j == 16 || j == 17 || j == 10 || j == 0) { continue; }
                        Get_theta(j, v_set, th_set);  //인덱스의 부모, 인덱스의 부모의 부모 참조해서 만드는 theta 값  ,radian 값         
                    ...
                    cout << "index 6 info :: normal v :" << nv_set.row(6) << "'\n' theta : " << th_set[6] << '\n';
                    cout << "syncronized left shoulder-elbow-wrist vector is:" << (nv_set.row(6) * th_set[6])* inv_rot_set[4] * inv_rot_set[2] * inv_rot_set[1] << "\n";
                ...
                //for (int i = 0; i <= 27; i++) {
                //    Get_theta(i, v_set, th_set);
                //    Get_n_vector(i, v_set, nv_set);
                //    Get_rotation_matrix(i, nv_set, th_set, rot_set[i]);
                //}
