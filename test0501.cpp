...
int main()
{
    ...
    VERIFY(k4a_device_open(0, &master_device), "Open master K4A Device failed!");
    VERIFY(k4a_device_open(0, &sub_device), "Open sub K4A Device failed!");

    // Start camera. Make sure depth camera is enabled.
    k4a_device_configuration_t m_deviceConfig = K4A_DEVICE_CONFIG_INIT_DISABLE_ALL;
    m_deviceConfig.color_format = K4A_IMAGE_FORMAT_COLOR_BGRA32;
    m_deviceConfig.camera_fps = K4A_FRAMES_PER_SECOND_5;
    m_deviceConfig.depth_mode = K4A_DEPTH_MODE_NFOV_UNBINNED;
    m_deviceConfig.color_resolution = K4A_COLOR_RESOLUTION_720P;
    VERIFY(k4a_device_start_cameras(master_device, &deviceConfig), "Start K4A cameras failed!");

    k4a_calibration_t m_sensor_calibration;
    VERIFY(k4a_device_get_calibration(master_device, deviceConfig.depth_mode, deviceConfig.color_resolution, &sensor_calibration),
            ...
            if (pop_frame_result == K4A_WAIT_RESULT_SUCCEEDED)
            {
                ...
                for (size_t i = 0; i < num_bodies; i++)
                {
                    ...
                    for (int j = 0; j <= 27; j++) {
                        ...
                        /*cv::line(colorMat, Point(xy_color[j].xy.x, xy_color[j].xy.y), Point(xy_color[joints_hierarchy[j]].xy.x, xy_color[joints_hierarchy[j]].xy.y), colors[j], 3, 1, 0);*/
                    }
                    ...
                    ////프레임 만들기
                 /*   int imgpts = projectPoints(axis, rvecs, tvecs, mtx, dist);*/


                    k4abt_joint_confidence_level_t confidence_level[28];
                    for (i = 0; i < 28; i++) {
                        confidence_level[i] = skeleton.joints[i].confidence_level;
                        ...
                        else if (skeleton.joints[i].confidence_level == K4ABT_JOINT_CONFIDENCE_HIGH) {
