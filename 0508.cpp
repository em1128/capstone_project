...
int vsize[2] = { 56, 3 }; // vector size
int rsize[3] = { 56, 3 ,3 }; //matrix size
// 추가 0407
int vsize_2D[2] = { 56,2 }; // vector 2D size
Scalar colors[28];
...
int main()
{
    ...
    VERIFY(k4a_device_open(1, &device2), "Open K4A Device failed!");
    ...
    do
    {
            ...
            if (pop_frame_result == K4A_WAIT_RESULT_SUCCEEDED && pop_frame_result2 == K4A_WAIT_RESULT_SUCCEEDED)
            {
                ...
                if (num_bodies == num_bodies2 && num_bodies != 0) {
                    ...
                    for (size_t i_body = 0; i_body < num_bodies; i_body++)
                    {
                        ...
                        cv::cvtColor(colorMat2, grayMat2, COLOR_BGR2GRAY);

                        // calibration & draw circle
                        for (int i = 0; i <= 27; i++) {
                        ...
                        for (int i = 0; i <= 27; i++) {
                            if (i == 9 || i == 16 || i == 17 || i == 10) { continue; }
                            int num_j = i;
                            float x = skeleton2[i_body].joints[num_j].position.xyz.x;
                            float y = skeleton2[i_body].joints[num_j].position.xyz.y;
                            float z = skeleton2[i_body].joints[num_j].position.xyz.z; // 24*3 
                            p_set2.at<float>(28 * i_body + i, 0) = x;
                            p_set2.at<float>(28 * i_body + i, 1) = y;
                            p_set2.at<float>(28 * i_body + i, 2) = z;

                            int re2 = 0;
                            k4a_calibration_3d_to_2d(&sensor_calibration2, &skeleton2[i_body].joints[num_j].position, K4A_CALIBRATION_TYPE_DEPTH, K4A_CALIBRATION_TYPE_COLOR, &xy_color2[28 * i_body + i], &re2);
                            //0407 추가
                            p_set_2D2.at<float>(28 * i_body + i, 0) = xy_color2[28 * i_body + i].xy.x;
                            p_set_2D2.at<float>(28 * i_body + i, 1) = xy_color2[28 * i_body + i].xy.y;
                            //

                            //Print3p(skeleton.joints[8].position.xyz.x, skeleton.joints[8].position.xyz.y, skeleton.joints[8].position.xyz.z);
                            //Print2p(xy_color.xy.x, xy_color.xy.y);
                            uint32_t id2 = k4abt_frame_get_body_id(body_frame2, i);
                            // joint xyz print
                            //cout << i <<"xyz is x:" << skeleton.joints[i].position.xyz.x << "y:" << skeleton.joints[i].position.xyz.y << "z:" << skeleton.joints[i].position.xyz.z << '\n';                   
                            if (color_image2 != NULL) {
                                if (color_buffer2 != NULL) {
                                    cv::circle(colorMat2, Point(xy_color2[28 * i_body + i].xy.x, xy_color2[28 * i_body + i].xy.y), 7, Scalar(255, 0, 0), 4, 1, 0);
                                }
                            }
                ...
                k4abt_frame_release(body_frame2); // Remember to release the body frame once you finish using it
            }
            else if (pop_frame_result == K4A_WAIT_RESULT_TIMEOUT || pop_frame_result2 == K4A_WAIT_RESULT_TIMEOUT)
            {
                //  It should never hit timeout when K4A_WAIT_INFINITE is set.
        ...
        else if (get_capture_result == K4A_WAIT_RESULT_TIMEOUT || get_capture_result2 == K4A_WAIT_RESULT_TIMEOUT)
        {
        ...
        {
            printf("Get depth capture returned error: %d and %d\n", get_capture_result, get_capture_result2);
            break;
        }
