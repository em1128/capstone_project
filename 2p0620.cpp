...
static void  Drawing_circle(vector<k4abt_skeleton_t> skeleton, int i_body, Mat& p_set, k4a_calibration_t sensor_calibration, k4a_float2_t xy_color[], Mat & p_set_2D,
k4abt_frame_t body_frame, Mat & colorMat , k4a_image_t color_image, uint8_t * color_buffer);
...
int main()
{
    ...
    const int32_t TIMEOUT_IN_MS = 100000000;
            ...
            if (pop_frame_result == K4A_WAIT_RESULT_SUCCEEDED && pop_frame_result2 == K4A_WAIT_RESULT_SUCCEEDED)
            {
                ...
                if (num_bodies == num_bodies2 && num_bodies != 0) {
                    ...
                    for (size_t i_body = 0; i_body < num_bodies; i_body++)
                    {
                        ...
                        Drawing_circle(skeleton, i_body, p_set, sensor_calibration, xy_color, p_set_2D, body_frame, colorMat, color_image,color_buffer);
						Drawing_circle(skeleton2, i_body, p_set2, sensor_calibration2, xy_color2, p_set_2D2, body_frame2, colorMat2, color_image2, color_buffer2);
...
static void  Drawing_circle(vector<k4abt_skeleton_t> skeleton, int i_body, Mat& p_set, k4a_calibration_t sensor_calibration, k4a_float2_t xy_color[], Mat& p_set_2D, k4abt_frame_t body_frame, Mat& colorMat, 
	k4a_image_t color_image, uint8_t* color_buffer) {
    for (int i = 0; i <= 27; i++) {
